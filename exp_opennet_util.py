  ...
  #  ws = np.random.randn(n_rots, n_dims, n_dims)
   # bs_train = np.random.randn(n_rots, n_train, n_dims)
    ...
    for i in range(1, n_trans+1):
        ...
        y_train_trans = np.concatenate([y_train_trans, y_train])
        decouple = np.concatenate([decouple, [i]*n_train])
    decouple_rot_one_hot = one_hot_convert(decouple)
    return x_train_trans, y_train_trans, train_rot, decouple_rot_one_hot

def get_start_candidates(x_train):
    islands = set(x_train[0])
    for i in range(1, len(x_train)):
        A = x_train[i].reshape(63, 63)
        islands = islands.intersection(set(np.where((~A.any(axis=0)) & (~A.any(axis=1)))[0].flatten()))
    return set(range(63)) - islands
...
def random_walk(a, i, iters):
    ...
    for k in range(iters):
      #  print(visit)
        start = np.random.choice(list(visit))
        probs = a[start] # probability of transitions
        if all(v == 0 for v in probs):
            continue
        probs_normal = probs / np.sum(probs)
        sample = np.random.choice(elements,p=probs_normal) # sample a target using probs
     #   while sample in visit:
       #     sample = np.random.choice(elements,p=probs_normal) # sample a target using probs
        visit.add(sample)
     #   print('keep nodes: ', visit)
    return visit

def ae_subgraph_transformation(x_train, y_train, x_dim, n_trans, iters=1000):
    start_candidates = get_start_candidates(x_train)
    random.seed(datetime.now())
    train_rot = x_train
    x_train_trans = x_train
    y_train_trans = y_train
    candidates = random.sample(start_candidates, n_trans)
    for start in candidates:
        print("start from: ", start)
       # rand_r = random.sample(range(0,x_dim), n_drop)
        for x in x_train:
            x_reshape = x.reshape(x_dim, x_dim)
            keep_nodes = random_walk(x_reshape, start, iters)
            remove_nodes = set(range(63)) - keep_nodes
            for n in remove_nodes:
                x_reshape[n] = 0
                x_reshape[:, n] = 0
            x_trans = x_reshape.reshape(1,x_dim*x_dim)
            x_train_trans = np.concatenate([x_train_trans, x_trans])
        train_rot = np.concatenate([train_rot, x_train])
        y_train_trans = np.concatenate([y_train_trans, y_train])
    return x_train_trans, y_train_trans, train_rot

def ae_edge_drop_transformation(x_train, y_train, x_dim, n_trans, percent=0.02):
    train_rot = x_train
    x_train_trans = x_train
    y_train_trans = y_train
    n_drop = int(x_dim * x_dim * percent)
    for i in range(n_trans):
        rand_r = random.sample(range(0, x_dim*x_dim), n_drop)
        for x in x_train:
            x_trans = x.copy()
            x_trans[rand_r] = 0
            x_train_trans = np.concatenate([x_train_trans, x_trans])
...
def ae_node_drop_transformation(x_train, y_train, x_dim, n_trans, percent=0.02):
    train_rot = x_train
    x_train_trans = x_train
    y_train_trans = y_train
    n_drop = int(x_dim * percent)
    random.seed(datetime.now())
...
def ae_node_drop_random_transformation(x_train, y_train, x_dim, n_trans):
    n_train, n_dims = x_train.shape
    x_train_trans = x_train
    train_rot = x_train
    #  x_train_trans = x_train
    ...
    random.seed(datetime.now())
    for i in range(n_trans):
        rand_drop = random.sample(range(0, x_dim), 1)
        rand_random = random.sample(r, len(r))
        for k, x in enumerate(x_train):
            x_drop = x.reshape(x_dim, x_dim)
            x_drop[rand_drop] = 0
            x_drop[:, rand_drop] = 0
            x_trans = x_drop[np.ix_(rand_random, rand_random)].reshape(1,x_dim*x_dim)
            x_train_trans = np.concatenate([x_train_trans, x_trans])
        train_rot = np.concatenate([train_rot, x_train])
        y_train_trans = np.concatenate([y_train_trans, y_train])
# train_rot_one_hot = one_hot_convert(train_rot)
...
def ae_graph_misc_tranfomration(x_train, y_train, x_dim):
    x_train_trans = []
    train_rot = []
    #  x_train_trans = x_train
    y_train_trans = []
   ...
   #     g = np.random.normal(mean, sigma, (1, x_dim*x_dim))
        rand_drop = random.sample(range(0, x_dim), 1)
        rand_shift = random.sample(range(1, x_dim), 1)
        rand_random = random.sample(r, len(r))
        x_drop = x.reshape(x_dim, x_dim)
        x_drop[rand_drop] = 0
        x_drop[:, rand_drop] = 0
        x_trans_1 = x_drop.reshape(1,x_dim*x_dim)
        x_t = np.roll(x.reshape(x_dim, x_dim), rand_shift, 0)
        x_trans_2 = np.roll(x_t, rand_shift, 1).reshape(1, x_dim * x_dim)
        x_trans_3 = x.reshape(x_dim,x_dim)[np.ix_(rand_random, rand_random)].reshape(1,x_dim*x_dim)
        x_trans_4 = x.reshape(1,x_dim*x_dim)
...
def ae_shift_transformation(x_train, y_train, x_dim, n_trans):
  ...
  #  ws = np.random.randn(n_rots, n_dims, n_dims)
   ...
   # x_train_trans = []
  #  for k, x in enumerate(x_train):
 #       x_t = np.roll(x.reshape(x_dim, x_dim), 8, 0)
  #      x_trans = np.roll(x_t, 8, 1).reshape(x_dim * x_dim)
  #      x_train_trans.append(x_trans)
  #  x_train_trans = np.array(x_train_trans)
 #   print("x train shape: ", x_train_trans.shape)
  ...
  #  x_train_trans = x_train
    ...
    decouple = [0] * n_train
    for i in range(1, n_trans+1):
        num = random.sample(range(1, x_dim), 1)
        print(num)
        for k, x in enumerate(x_train):
...
def ae_rotate_gaussian_transformation(x_train, y_train, x_dim, n_trans):
    n_train, n_dims = x_train.shape
    train_rot = x_train
    x_train_trans = x_train
    y_train_trans = y_train
    ...
    for i in range(1, n_trans + 1):
        ...
        for k, x in enumerate(x_train):
            x_trans = np.rot90(x.reshape(x_dim, x_dim), i).reshape(1, x_dim*x_dim) + g
            x_train_trans = np.concatenate([x_train_trans, x_trans])
        train_rot = np.concatenate([train_rot, x_train])
        y_train_trans = np.concatenate([y_train_trans, y_train])
   # train_rot_one_hot = one_hot_convert(train_rot)
...
def swap_transformation(x_train, y_train, x_dim, n_trans):
    n_train, n_dims = x_train.shape
  #  ws = np.random.randn(n_rots, n_dims, n_dims)
   # bs_train = np.random.randn(n_rots, n_train, n_dims)
    train_rot = [0] * n_train
...
def concat_labels(xs, label):
    new_xs_full = []
    for i in range(len(label)):
        label_i = label[i].reshape((1, 63))
        xs_reshape = xs[i].reshape((63,63))
        new_xs= np.concatenate((label_i, xs_reshape), axis=0)
        new_label = np.concatenate((np.zeros((1,1), dtype=int), label_i), axis=1)
        new_fcg_xs = np.concatenate((new_label.T, new_xs), axis=1)
        new_xs_full.append(new_fcg_xs.flatten())
    return np.array(new_xs_full)
...
def load_open_dataset(dataset_name, tr_classes, seed, rot=None, normalize=False):
    if dataset_name == 'ms':
        ...
        return OpenWorldMsData(fcg_adj_xs, np.argmax(fcg_adj_ys, axis=1),
                               tr_classes=tr_classes, comb_val_test=False, seed=1, normalize=normalize)
       ...
       # with open('./data/pickle/android_malware_genome_project_5-bit-cluster_32-bit_minhash-1_1-gram.pkl') as fin:
        ...
        android_fcg_xs = np.load('./data/Android/android_training_data_normalized_full_allnorm.npy')
        android_fcg_ys = np.load('./data/Android/android_training_labels_full.npy')
        android_vertices = np.load('./data/Android/android_training_vertices_normalized_full_allnorm.npy')

   #     android_fcg_xs = concat_labels(android_fcg_xs_orig, android_vertices)

     #   idx = np.argwhere(np.all(android_fcg_xs[..., :] == 0, axis=0))
     #   android_fcg_xs = np.delete(android_fcg_xs, idx, axis=1)
    #    android_fcg_xs = np.concatenate((android_vertices, android_fcg_xs_orig), axis=1)
    ...
    #    android_fcg_xs = np.reshape(android_fcg_xs, [android_fcg_xs.shape[0], reduce(lambda x, y: x * y, android_fcg_xs.shape[1:])])
     #   enc = preprocessing.OneHotEncoder(n_values=9, sparse=False)
     #   enc.fit(android_fcg_ys.reshape(-1, 1))
     #   android_fcg_ys = enc.transform(android_fcg_ys.reshape(-1, 1))
    ...
    elif dataset_name == 'colors':
        x_train = np.load('data/COLORS-3/colors_training_data.npy')
        y_train = np.load('data/COLORS-3/colors_training_labels.npy')
        return OpenWorldMsData(x_train, y_train,
                               tr_classes=tr_classes,
                               comb_val_test=False, seed=seed, normalize=normalize)

    elif dataset_name == 'triangles':
        x_train = np.load('data/TRIANGLES/triangles_training_data.npy')
        y_train = np.load('data/TRIANGLES/triangles_training_labels.npy')
        return OpenWorldMsData(x_train, y_train,
                               tr_classes=tr_classes,
                               comb_val_test=False, seed=seed, normalize=normalize)
