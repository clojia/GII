...
def open_set_classification_metric(true_y, pred_y, is_openset=True, acc=None, label_text_lookup=None, save_path_heatmap=None):
  ...
  #  ms_labels = ['Lollipop', 'Kelihos ver3', 'Vundo', 'Tracur', 'Kelihos ver1', 'Obfuscator.ACY', 'Unknown']
    cm = metrics.confusion_matrix(true_y, pred_y)
    cmn = cm.astype('float') / cm.sum(axis=1)[:, np.newaxis]
    plt.figure()
    ax = plt.subplot()
    sns.heatmap(cmn, annot=True, fmt='.2f', xticklabels=cm_labels, yticklabels=cm_labels, ax=ax)  # annot=True to annotate cells, ftm='g' to disable scientific notation
