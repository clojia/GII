...
def visualize_t_SNE_for_train(X, ys, X_unknown, ys_unknown,  ts_known_mask=None, grid_shape=(2,2), alpha=0.5, xlim=None, ylim=None,
                    loc='upper left', bbox_to_anchor=(1.04,1), figsize=(5, 5),
                    unique_ys=None, save_path=None, label_text_lookup=None, tr_classes=None):

    plt.figure()
  #  print("t sne")
   # print(len(X), len(ys))
    labels = []
    y_dim = len(np.unique(ys))
  #  n_cluster = len(np.unique(ys_unknown))
  #  print('ys ', ys)
  #  print('ys unknown ', ys_unknown)
    ys_full = np.concatenate((ys, ys_unknown + y_dim))
  #  ys_full = np.concatenate((yst, y_unknownt))
    for label in ys_full:
        if label < len(tr_classes):
            labels.append(label_text_lookup[tr_classes[label]])
        else:
   ...
   # palette = sns.color_palette("bright", len(set(labels)))
    ...
    tsne = TSNE(n_components=2, verbose=0, perplexity=30, random_state=1, n_iter=1000)
    ...
    tsne_df = pd.DataFrame({'t0': tsne_results[:, 0],
                            't1': tsne_results[:, 1],
                            'Category': labels,
                            'labeled': [True] * len(X) + [False] * len(X_unknown)
                            })

    plt.figure()
    hue_order = sorted(set(labels))
  #  s = sns.FacetGrid(tsne_df, hue="Category", hue_order=hue_order, height=4)
    s = sns.FacetGrid(tsne_df, hue="Category", hue_order=hue_order, col="labeled", col_order=[True, False], height=4)
    s.map(plt.scatter, "t0", "t1", alpha=.5, linewidth=.3, edgecolor="white")
 #   s.set(xlim=(-75, 75), ylim=(-75, 75))
    s.add_legend()
    print("here")
    if save_path:
...
def visualize_t_SNE(X, ys, ts_known_mask=None, grid_shape=(2,2), alpha=0.5, xlim=None, ylim=None,
                    loc='upper left', bbox_to_anchor=(1.04,1), figsize=(5, 5),
                    unique_ys=None, save_path=None, label_text_lookup=None, tr_classes=None, n_clusters=2,
                    tr_cluster_mask=None):

    plt.figure()
    labels = []
    for label in ys:
        if label < len(tr_classes) - n_clusters:
            labels.append(label_text_lookup[tr_classes[label]])
        elif label < len(tr_classes):
            labels.append('Cluster ' + str(len(tr_classes)-label))
        else:
            labels.append('Unknown')

    cluster_mask = [False] * len(ys)

    dataset = np.array(['Labeled'] * len(ys))
   # print(tr_cluster_mask)
    dataset[tr_cluster_mask] = 'Unlabeled'
    dataset[np.logical_not(ts_known_mask)] = 'Unknown'

   # c_lookup = gen_color_map(label_text_lookup.values())
   # palette = sns.color_palette("bright", len(set(labels)))

    time_start = time.time()
    tsne = TSNE(n_components=2, verbose=0, perplexity=50, random_state=0)
    ...
    plt.figure()
    hue_order = sorted(list(set(labels)))
    hue_order.remove('Unknown')
    for i in range(1, n_clusters+1):
        hue_order.remove('Cluster ' + str(i))
    for i in range(1, n_clusters+1):
        hue_order += ['Cluster ' + str(i)]
    hue_order += ['Unknown']
    s = sns.FacetGrid(tsne_df, hue="Category", hue_order=hue_order, col="Known", col_order=[True, False], height=4)
    s.map(plt.scatter, "t0", "t1", alpha=.5, linewidth=.3, edgecolor="white")
    s.set(xlim=(-60, 60), ylim=(-60, 60))
