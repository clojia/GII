...
class OpenWorldSim(object):
    ...
    def __init__(self, train_data, train_lable, train_rot=None,
                 ...
                 tr_classes=6, validation_classes=[], n_cluster=2, seed=None):
        np.random.seed(seed)
        ...
        if isinstance(tr_classes, list):
            ...
            self.known_classes = tr_classes[:-1 * n_cluster]
            self.cluster_classes = tr_classes[-1 * n_cluster:]
        ...
        self.tr_k_mask = self.mask_unknown(train_lable, self.known_classes)
        self.val_k_mask = self.mask_unknown(val_label, self.known_classes)
        self.ts_k_mask = self.mask_unknown(test_label, self.known_classes)

        self.tr_cluster_mask = self.mask_unknown(train_lable, self.cluster_classes)
        self.val_cluster_mask = self.mask_unknown(val_label, self.cluster_classes)
        self.ts_cluster_mask = self.mask_unknown(test_label, self.cluster_classes)

        self.tr_unk_mask = self.mask_unknown(train_lable, self.unknown_classes)
        self.val_unk_mask = self.mask_unknown(val_label, self.unknown_classes)
        self.ts_unk_mask = self.mask_unknown(test_label, self.unknown_classes)
    ...
    def _del_unknown_class(self, label, classes):
        return np.delete(label, classes, axis=1)

    def train_label_ssl(self):
        if len(self.tr_lable.shape) == 1:
            label = self.tr_lable[self.tr_mask]
        else:
            label = self.tr_lable[self.tr_mask]
        #  return label
        return self._del_unknown_class(label, self.unknown_classes)

    def train_label(self):
        if len(self.tr_lable.shape) == 1:
            label = self.tr_lable[self.tr_k_mask]
        else:
            label = self.tr_lable[self.tr_k_mask]
      #  return label
        return self._del_unknown_class(label, self.unknown_classes + self.cluster_classes)

    def train_label_unknown(self):
     #   unknown_mask = np.logical_not(self.tr_mask)
        if len(self.tr_lable.shape) == 1:
            label = self.tr_lable[self.tr_cluster_mask]
        else:
            label = self.tr_lable[self.tr_cluster_mask]
       # return label
       # print('del ', self.unknown_classes + self.known_classes)
        return self._del_unknown_class(label, self.unknown_classes + self.known_classes)
    ...
    def train_data_ssl(self):
        return self.tr_data[self.tr_mask]

    def train_data(self):
        return self.tr_data[self.tr_k_mask]

    def train_data_unknown(self):
        return self.tr_data[self.tr_cluster_mask]
    ...
    def _val_test(self, label, mask):
        if len(label.shape) == 1:
            ...
            cluster = label[:, self.cluster_classes]
            label = np.concatenate((label, cluster, unknown), axis=1)
       # return label
        return self._del_unknown_class(label, self.unknown_classes + self.cluster_classes)

    def validation_label(self):
        label = np.copy(self.val_label)
        return self._val_test(label, self.val_mask)

    def validation_data(self):
        return self.val_data

    def test_label(self):
        label = np.copy(self.ts_label)
        return self._val_test(label, self.ts_mask)
...
class OpenWorldMsData(object):
    ...
    def __init__(self, data, label, train_rot=None,
                 tr_classes=6, val_classes=[], comb_val_test=False, seed=None, n_cluster=2,
                 normalize=False):
        """
        ...
        self.tr_mask = self.mask_unknown(tr_y, self.tr_classes)
        self.val_mask = self.mask_unknown(val_y[:split_point], self.tr_classes)
        self.ts_mask = self.mask_unknown(ts_y[split_point:], self.tr_classes)

        self.tr_k_mask = self.mask_unknown(tr_y, self.known_classes)
        self.val_k_mask = self.mask_unknown(val_y[:split_point], self.known_classes)
        self.ts_k_mask = self.mask_unknown(ts_y[split_point:], self.known_classes)

        self.tr_cluster_mask = self.mask_unknown(tr_y, self.cluster_classes)
        self.val_cluster_mask = self.mask_unknown(val_y[:split_point], self.cluster_classes)
        self.ts_cluster_mask = self.mask_unknown(ts_y[split_point:], self.cluster_classes)

        self.tr_unk_mask = self.mask_unknown(tr_y, self.unknown_classes)
        self.val_unk_mask = self.mask_unknown(val_y[:split_point], self.unknown_classes)
        self.ts_unk_mask = self.mask_unknown(ts_y[split_point:], self.unknown_classes)

        self.tr_data = tr_x
        self.tr_label = enc.transform(tr_y.reshape(-1,1))
        self.val_data = ts_x[:split_point]
        self.val_label = enc.transform(ts_y[:split_point].reshape(-1,1))
        self.ts_data = ts_x[split_point:]
        self.ts_label = enc.transform(ts_y[split_point:].reshape(-1,1))

        # self.tr_data = preprocessing.normalize(tr_x) if normalize else tr_x
        # self.tr_label = self._del_unknown_class(enc.transform(tr_y.reshape(-1, 1)), self.unknown_classes+self.cluster_classes)
        #
    ...
    def train_label(self):
        label = self.tr_label[self.tr_k_mask]
        return self._del_unknown_class(label, self.unknown_classes + self.cluster_classes)

    def train_label_ssl(self):
        label = self.tr_label[self.tr_mask]
        return self._del_unknown_class(label, self.unknown_classes)

    def train_label_unknown(self):
        label = self.tr_label[self.tr_cluster_mask]
        return self._del_unknown_class(label, self.unknown_classes + self.known_classes)
